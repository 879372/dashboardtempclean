...
tab_card = {'height': '100%', 'margimTop':'10px'}
...
def cfop_filter(cfop):
    if isinstance(cfop, str): 
        cfop = [cfop]
    
    if 'Todos' in cfop:
        mask = df['CFOP'].notnull()
    else:
        mask = df['CFOP'].isin(cfop) 
    return mask

def vendedor_filter(vendedor):
    if isinstance(vendedor, str): 
        vendedor = [vendedor]
    
    if 'Todos' in vendedor:
        mask = df['VENDEDOR'].notnull()
    else:
        mask = df['VENDEDOR'].isin(vendedor)
    return mask
...
main_layout = dbc.Container(children=[
                ...
                dbc.CardBody([
                    dbc.Row(
                        dbc.Col([
                            ...
                            html.Div(id='year-select', style={'text-align': 'center', 'margin-top': '10px'}, className='dbc'),
                            ...
                            html.Div(id='month-select', style={'text-align': 'center', 'margin-top': '10px'}, className='dbc'),
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H4('Distribuição de Faturamento por Cliente'),
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H4("Faturamento Mensal por Cliente"),
...
@app.callback(
    ...
    Input('radio-cfop', 'value'),
    Input('radio-vendedor', 'value'),
    Input(ThemeSwitchAIO.ids.switch("theme"), "value"),
    Input('interval-component', 'n_intervals')  
)
def update_graph1e2(month, year, team, cfop, vendedor, toggle, n_intervals):
    with lock:
        try:
            ...
            df_1 = df_filtered.groupby(['VENDEDOR'])['VALOR_PAGO'].sum().reset_index()
            df_1 = df_1.groupby('VENDEDOR').head(1).reset_index()
            df_1 = df_1.sort_values(by='VALOR_PAGO', ascending=False)

            df_1['VENDEDOR'] # Combine os rótulos
            df_1['TOTAL_VENDAS'] = df_1['VALOR_PAGO'].map(formatar_reais)
            
            fig1 = go.Figure(go.Bar(x=df_1['VENDEDOR'], y=df_1['VALOR_PAGO'], text=df_1['TOTAL_VENDAS'], textposition='auto'))
            fig1.update_layout(main_config, height=250, template=template, showlegend=False,)

            # Gráfico 2: sem donut hole
            fig2 = go.Figure(go.Pie(labels=df_1['VENDEDOR'], values=df_1['VALOR_PAGO'], hole=.6))
            fig2.update_layout(main_config, height=230, template=template, showlegend=False)
        except Exception as e:
            print(f"Erro ao obter dados do graph 1 e 2: {e}")
            # Se ocorrer um erro, retorne gráficos vazios ou uma mensagem de erro
            fig1 = go.Figure()
            fig2 = go.Figure()
        return fig1, fig2
...
def update_graph3( month, year, team, cfop, vendedor, toggle, n_intervals):
    with lock:
        try:
            template = template_theme1 if toggle else template_theme2
            global df
...
def update_graph4( month, year, team, cfop, vendedor, toggle, n_intervals):
    with lock:
        try:
            template = template_theme1 if toggle else template_theme2
            global df
            ...
            mask_cfop = cfop_filter(cfop)
            mask_vendedor = vendedor_filter(vendedor)  # Aplicando o filtro de vendedor
            df_team_zero = df.loc[mask_zero & mask_team & mask_cfop & mask_vendedor]
...
def update_graph5( month, year, team, cfop, toggle,  n_intervals):
    with lock:
        try:
            template = template_theme1 if toggle else template_theme2
            global df
            ...
            mask_team = team_filter(team)
            mask_cfop = cfop_filter(cfop)
            df_filtered = df.loc[mask_year & mask_month & mask_team & mask_cfop]
...
def update_graph6( month, year, team, cfop, toggle, n_intervals):
    with lock:
        try:
            template = template_theme1 if toggle else template_theme2
            global df
...
def update_graph7( month, year, team, cfop, vendedor, toggle, n_intervals):
    with lock:
        try:
            template = template_theme1 if toggle else template_theme2
            global df
            ...
            df_zero = df.loc[mask_zero & mask_cfop & mask_vendedor]
...
def update_graph8( month, year, team, cfop, vendedor, toggle, n_intervals):
    with lock:
        try:
            template = template_theme1 if toggle else template_theme2
            global df
...
def update_graph9( month, year, team, cfop, vendedor, toggle, n_intervals):
    with lock:
        try:
            template = template_theme1 if toggle else template_theme2
            global df
            ...
            mask_cfop = cfop_filter(cfop)
            mask_vendedor = vendedor_filter(vendedor)  # Aplicando o filtro de vendedor
            df_meseano = df.loc[mask_year & mask_month & mask_cfop & mask_vendedor]
...
def update_graph10( month, year, team, cfop, vendedor, toggle, n_intervals):
    with lock:
        try:
            template = template_theme1 if toggle else template_theme2
            global df
...
def update_graph11( month, year, team, cfop, vendedor, toggle, n_intervals):
    with lock:
        try:
            template = template_theme1 if toggle else template_theme2
            global df
...
def update_graph12( month, year, team, cfop, vendedor, toggle, n_intervals):
    with lock:
        try:
            template = template_theme1 if toggle else template_theme2
            global df
...
def update_graph13e14(month, year, team, cfop, vendedor, toggle, n_intervals):
    with lock:
        try:
            template = template_theme1 if toggle else template_theme2
            global df
            ...
            fig13 = go.Figure(go.Bar(x=df_1['CLIENTE'], y=df_1['VALOR_PAGO'], text=df_1['TOTAL_VENDAS'], textposition='auto'))
            fig13.update_layout(main_config, height=250, template=template, showlegend=False)

            # Gráfico 2: sem donut hole
            fig14 = go.Figure(go.Pie(labels=df_1['CLIENTE'], values=df_1['VALOR_PAGO'], hole=.6))
            fig14.update_layout(main_config, height=230, template=template, showlegend=False)
        except Exception as e:
            print(f"Erro ao obter dados do graph 13 e 14: {e}")
            # Se ocorrer um erro, retorne gráficos vazios ou uma mensagem de erro
            fig13 = go.Figure()
            fig14 = go.Figure()
        return fig13, fig14
...
def update_dropdowns(n_intervals, selected_year):
    try:
        mes_atual = datetime.datetime.now().month
        ano_atual = datetime.datetime.now().year
        ...
        if selected_year:
            ...
            options_month = [{'label': convert_to_text(i), 'value': i} for i in df_filtered['MES'].unique()]
            options_month = sorted(options_month, key=lambda x: x['value'])

            default_month = mes_atual if selected_year == ano_atual else options_month[0]['value']
        else:
            options_month = []
            default_month = None

        options_team = [{'label': 'Todas as Empresas', 'value': 0}] + [{'label': i, 'value': i} for i in df['EMPRESA'].unique()]

    except Exception as e:
        print(f"Erro ao obter dados: {e}")
        options_year = options_month = options_team = []
        selected_year = default_month = 0

    return options_year, selected_year, options_month, default_month, options_team, 0

@app.callback(
    Output('radio-vendedor', 'options'),
    Output('radio-vendedor', 'value'),
    Input('interval-component', 'n_intervals')
)
def update_vendedor_options(n_intervals):
    global df
    vendedores = df['VENDEDOR'].unique()
    options = [{'label': 'Todos', 'value': 'Todos'}] + [{'label': vendedor, 'value': vendedor} for vendedor in vendedores]
    return options, ['Todos']


@app.callback(
    Output("radio-cfop", "options"),
    Output("radio-cfop", "value"),
    Input('interval-component', 'n_intervals'),
)
